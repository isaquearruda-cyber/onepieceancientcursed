(function () {
	...
	function instalarPwa() {
		...
		if ("serviceWorker" in navigator && location.protocol !== "file:") {
			navigator.serviceWorker.register("service-worker.js?v=20260705-unlock1", { updateViaCache: "none" }).then(function (registro) {
				registro.update();
	...
	function instalarCss() {
		...
		style.textContent = [
			...
			".rpg-loading{position:fixed;inset:0;z-index:99999;display:grid;place-items:center;padding:18px;background:radial-gradient(circle at 50% 30%,rgba(255,226,138,.18),transparent 34%),radial-gradient(circle at 18% 72%,rgba(255,65,40,.18),transparent 26%),linear-gradient(180deg,#020707,#071314 58%,#160806);color:#fff7df;transition:opacity .36s ease,visibility .36s ease;overflow:hidden}",
			".rpg-loading::before{content:'';position:absolute;inset:0;background:linear-gradient(90deg,rgba(255,255,255,.035) 1px,transparent 1px),linear-gradient(0deg,rgba(255,255,255,.028) 1px,transparent 1px);background-size:48px 48px,48px 48px;mask-image:radial-gradient(circle at 50% 50%,#000 0,transparent 72%);animation:rpgGridMove 6s linear infinite}.rpg-loading::after{content:'';position:absolute;inset:0;background:repeating-linear-gradient(0deg,rgba(255,255,255,.045) 0 1px,transparent 1px 6px);mix-blend-mode:screen;opacity:.24;pointer-events:none}",
			".rpg-loading.saindo{opacity:0;visibility:hidden}.rpg-loading-card{position:relative;z-index:1;width:min(760px,100%);display:grid;grid-template-columns:minmax(160px,.75fr) minmax(260px,1fr);gap:clamp(16px,4vw,34px);align-items:center;padding:clamp(16px,3vw,26px);border:1px solid rgba(255,226,138,.28);border-radius:8px;background:linear-gradient(135deg,rgba(5,15,18,.78),rgba(2,6,8,.56));box-shadow:0 30px 100px rgba(0,0,0,.72),inset 0 0 34px rgba(255,247,223,.06);backdrop-filter:blur(8px)}.rpg-loading-logo{position:relative;width:min(220px,44vw);aspect-ratio:443/498;margin:auto;background:url('assets/img/loading/luffy-loader.gif') center/contain no-repeat;filter:drop-shadow(0 24px 24px rgba(0,0,0,.58)) drop-shadow(0 0 20px rgba(255,226,138,.18));image-rendering:auto;animation:rpgLuffyRun 1.15s ease-in-out infinite}.rpg-loading-logo::after{content:'';position:absolute;inset:auto 8% -6% 8%;height:28px;border-radius:999px;background:radial-gradient(ellipse at center,rgba(0,0,0,.62),transparent 68%);filter:blur(2px)}",
			".rpg-loading-info{display:grid;gap:12px;text-align:left}.rpg-loading h2{margin:0;font-family:'Segoe UI Black','Arial Black',Impact,sans-serif;font-size:clamp(2rem,7vw,4.6rem);line-height:.86;text-transform:uppercase;text-shadow:0 5px 0 #210b08,0 18px 34px rgba(0,0,0,.72)}.rpg-loading small{color:#ffe28a;font-weight:1000;letter-spacing:.16em;text-transform:uppercase}.rpg-loading-barra{height:14px;border:1px solid rgba(255,226,138,.44);border-radius:999px;background:rgba(0,0,0,.52);overflow:hidden;box-shadow:inset 0 0 14px rgba(0,0,0,.5)}.rpg-loading-barra i{display:block;width:0;height:100%;border-radius:inherit;background:linear-gradient(90deg,#ff4128,#ffe28a,#90ff00,#36e0ff);box-shadow:0 0 24px rgba(255,226,138,.5);animation:rpgLoadBar 1.65s cubic-bezier(.19,1,.22,1) forwards}.rpg-loading-dica{min-height:42px;color:#d9e8d4;font:900 .95rem/1.35 Arial,sans-serif}.rpg-loading-etapas{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.rpg-loading-etapas span{padding:8px;border:1px solid rgba(255,247,223,.12);border-radius:8px;background:rgba(255,247,223,.045);color:#c9f7ff;font:1000 .64rem Arial,sans-serif;text-transform:uppercase;letter-spacing:.08em;text-align:center}.rpg-loading-etapas span:nth-child(1){animation:rpgEtapa .7s ease .18s both}.rpg-loading-etapas span:nth-child(2){animation:rpgEtapa .7s ease .52s both}.rpg-loading-etapas span:nth-child(3){animation:rpgEtapa .7s ease .86s both}",
			...
			"@keyframes rpgLuffyRun{50%{transform:translateY(-9px) rotate(-1deg)}}@keyframes rpgGridMove{to{transform:translate3d(-48px,-48px,0)}}@keyframes rpgEtapa{from{opacity:.35;transform:translateY(8px)}to{opacity:1;transform:none;border-color:rgba(144,255,0,.34);color:#eaffd8}}@keyframes rpgLoadPulse{50%{transform:translateY(-3px);box-shadow:0 0 38px rgba(247,200,75,.44)}}@keyframes rpgLoadBar{to{width:100%}}@keyframes rpgTransicaoIn{from{opacity:0}to{opacity:1}}",
			...
			".rpg-modal-sistema{position:fixed;inset:0;z-index:5200;display:grid;place-items:center;padding:16px;background:rgba(0,0,0,.68);backdrop-filter:blur(6px)}.rpg-modal-sistema[hidden]{display:none}.rpg-sistema-card{width:min(760px,100%);max-height:min(760px,calc(100svh - 28px));overflow:auto;border:1px solid rgba(255,226,138,.36);border-radius:8px;background:radial-gradient(circle at 16% 8%,rgba(54,224,255,.16),transparent 34%),linear-gradient(180deg,rgba(6,16,18,.98),rgba(1,4,6,.98));box-shadow:0 28px 90px rgba(0,0,0,.72);color:#fff7df}.rpg-sistema-topo{display:flex;justify-content:space-between;gap:12px;align-items:start;padding:18px;border-bottom:1px solid rgba(255,226,138,.16)}.rpg-sistema-topo small,.rpg-sistema-bloco h3{color:#90ff00;font-weight:1000;letter-spacing:.12em;text-transform:uppercase}.rpg-sistema-topo h2{margin:4px 0 0;font-family:'Segoe UI Black','Arial Black',Impact,sans-serif;font-size:clamp(1.8rem,6vw,3rem);line-height:.92;text-transform:uppercase;text-shadow:0 5px 0 #210b08,0 16px 30px rgba(0,0,0,.65)}.rpg-sistema-fechar{min-width:38px;min-height:38px;border:1px solid rgba(255,226,138,.36);border-radius:8px;background:rgba(3,8,10,.78);color:#fff7df;font-weight:1000;cursor:pointer}",
				...
				".rpg-page-rail{position:fixed;left:14px;top:14px;z-index:4380;display:grid;grid-template-columns:auto 1fr;gap:8px;align-items:center;max-width:min(520px,calc(100vw - 28px));padding:8px;border:1px solid rgba(255,226,138,.24);border-radius:8px;background:rgba(3,8,10,.72);box-shadow:0 14px 40px rgba(0,0,0,.38);backdrop-filter:blur(8px)}.rpg-page-rail strong{display:block;color:#ffe28a;font:1000 .72rem Arial,sans-serif;letter-spacing:.08em;text-transform:uppercase}.rpg-page-rail span{display:block;color:#d9e8d4;font:800 .7rem Arial,sans-serif}.rpg-page-links{display:flex;flex-wrap:wrap;gap:5px}.rpg-page-links a{display:grid;place-items:center;min-width:34px;height:32px;border:1px solid rgba(255,247,223,.14);border-radius:8px;background:rgba(255,247,223,.055);color:#fff7df;text-decoration:none;font:1000 .64rem Arial,sans-serif;text-transform:uppercase}.rpg-page-links a.ativo,.rpg-page-links a:hover{border-color:rgba(144,255,0,.42);background:rgba(144,255,0,.12);color:#eaffd8}",
			...
			".rpg-page-boost{--ui-panel:rgba(3,10,12,.72);--ui-border:rgba(255,226,138,.18);--ui-shadow:0 12px 28px rgba(0,0,0,.32)}.rpg-gameframe{border-color:rgba(255,226,138,.08)!important;box-shadow:inset 0 0 0 1px rgba(54,224,255,.035)!important}.rpg-gameframe::before,.rpg-gameframe::after{width:min(12vw,120px)!important;height:min(12vw,120px)!important;border-color:rgba(255,226,138,.16)!important}.rpg-game-topbar{top:8px!important;width:min(640px,calc(100vw - 24px))!important;min-height:38px!important;padding:6px 8px!important;gap:7px!important;background:var(--ui-panel)!important;border-color:var(--ui-border)!important;box-shadow:var(--ui-shadow)!important;backdrop-filter:blur(6px)!important}.rpg-game-topbar strong{font-size:.72rem!important;letter-spacing:.05em!important}.rpg-game-topbar span{font-size:.58rem!important;letter-spacing:.06em!important}.rpg-game-topbar button{min-height:26px!important;padding:0 10px!important}.rpg-minimap{width:104px!important;top:58px!important;right:10px!important;padding:7px!important;opacity:.72!important;background:rgba(3,10,12,.66)!important;border-color:rgba(54,224,255,.18)!important;box-shadow:var(--ui-shadow)!important;backdrop-filter:blur(5px)!important}.rpg-minimap strong{display:none!important}.rpg-minimap-dot{width:5px!important;height:5px!important;box-shadow:0 0 8px var(--cor,#ffe28a)!important}.rpg-minimap-grid::after{width:7px!important;height:7px!important}.rpg-quest-dock{top:auto!important;left:12px!important;bottom:58px!important;width:min(238px,calc(100vw - 86px))!important;gap:5px!important;padding:8px 9px!important;opacity:.82!important;background:var(--ui-panel)!important;border-color:var(--ui-border)!important;box-shadow:var(--ui-shadow)!important;backdrop-filter:blur(5px)!important}.rpg-quest-dock small{display:none!important}.rpg-quest-dock strong{font-size:.72rem!important;letter-spacing:.04em!important}.rpg-quest-dock span{display:none!important}.rpg-quest-bar{height:5px!important}.rpg-save-status{max-width:46px!important;height:36px!important;padding:0 10px!important;border-color:rgba(255,226,138,.16)!important;background:rgba(3,10,12,.62)!important;box-shadow:0 10px 22px rgba(0,0,0,.28)!important;overflow:hidden;color:transparent!important}.rpg-save-status i{flex:0 0 auto!important}.rpg-hotkeys,.rpg-versao-visivel{display:none!important}.rpg-toast{box-shadow:0 12px 28px rgba(0,0,0,.42)!important}.rpg-click-impact,.rpg-action-spark{display:none!important}.portal-mundo-lobby::before{display:none!important}.portal-mundo-lobby,.painel-proxima-acao,.battle-perfil,.battle-berris,.battle-xp{box-shadow:var(--ui-shadow)!important}.portal-mundo-motores{display:none!important}",
			".rpg-page-rail{max-width:44px!important;overflow:hidden!important;opacity:.72!important;transition:max-width .18s ease,opacity .18s ease!important}.rpg-page-rail:not(:hover):not(:focus-within)>div:first-child{display:none!important}.rpg-page-rail:not(:hover):not(:focus-within) .rpg-page-links a:not(.ativo){display:none!important}.rpg-page-rail:not(:hover):not(:focus-within) .rpg-page-links{display:grid!important;grid-template-columns:34px!important;padding:0!important}.rpg-page-rail:hover,.rpg-page-rail:focus-within{max-width:min(420px,calc(100vw - 24px))!important;opacity:1!important}.rpg-page-rail>div:first-child{min-width:190px!important}.rpg-page-links{min-width:max-content!important}.pagina-inventario .inventario-jogo{padding-top:64px!important}.pagina-inventario .inventario-topo{min-height:0!important}.pagina-inventario .inventario-topo h1{font-size:clamp(1.45rem,3vw,2.2rem)!important}.pagina-inventario .inventario-shell{max-height:calc(100dvh - 132px)!important}",
	...
	function instalarSeloVersaoNova() {
		...
		selo.innerHTML = "<i></i><span>Atualização V34 carregada</span>";
	...
	function carregarBibliotecasCdn() {
		...
		carregarCssUmaVez("https://cdn.jsdelivr.net/npm/sweetalert2@11/dist/sweetalert2.min.css", "rpg-sweetalert-css");
		carregarCssUmaVez("https://unpkg.com/tippy.js@6/dist/tippy.css", "rpg-tippy-css");
		Promise.allSettled([
			carregarScriptUmaVez("https://cdn.jsdelivr.net/npm/gsap@3.13.0/dist/gsap.min.js", "rpg-gsap-cdn"),
			...
			carregarScriptUmaVez("https://cdn.jsdelivr.net/npm/lenis@1.3.4/dist/lenis.min.js", "rpg-lenis-cdn"),
				...
				carregarScriptUmaVez("https://cdnjs.cloudflare.com/ajax/libs/vanilla-tilt/1.8.1/vanilla-tilt.min.js", "rpg-tilt-cdn")
			]).then(function () {
			cdnPremium.gsap = Boolean(window.gsap);
			cdnPremium.particles = typeof window.particlesJS === "function";
			cdnPremium.sweetalert = Boolean(window.Swal);
			...
			console.info("Camada CDN premium indisponivel.", erro);
		});
	}

	function ativarParticlesCdn() {
		if (!cdnPremium.particles || document.getElementById("rpg-cdn-particles")) return;
		var alvo = document.createElement("div");
		alvo.id = "rpg-cdn-particles";
		document.body.prepend(alvo);
		window.particlesJS("rpg-cdn-particles", {
			particles: {
				number: { value: 78, density: { enable: true, value_area: 920 } },
				color: { value: ["#90ff00", "#36e0ff", "#ffe28a", "#ff6b35"] },
				shape: { type: "circle" },
				opacity: { value: 0.36, random: true },
				size: { value: 3.2, random: true },
				line_linked: { enable: true, distance: 126, color: "#36e0ff", opacity: 0.18, width: 1 },
				move: { enable: true, speed: 1.15, direction: "top", random: true, straight: false, out_mode: "out" }
			},
			interactivity: {
				detect_on: "canvas",
				events: { onhover: { enable: true, mode: "grab" }, onclick: { enable: true, mode: "push" }, resize: true },
				modes: { grab: { distance: 150, line_linked: { opacity: 0.3 } }, push: { particles_nb: 5 } }
			},
			retina_detect: true
		});
	}

	function ativarGsapPremium() {
		if (!cdnPremium.gsap) return;
		var gsap = window.gsap;
		gsap.fromTo(".battle-hud-lobby,.painel-proxima-acao,.rpg-passe-lobby", { y: 14, opacity: 0 }, { y: 0, opacity: 1, duration: 0.72, stagger: 0.08, ease: "power3.out", overwrite: "auto" });
		gsap.fromTo(".atalho-menu-lobby,.botao-lobby,.botao-menu-lobby", { scale: 0.985, opacity: 0.92 }, { scale: 1, opacity: 1, duration: 0.44, stagger: 0.025, ease: "back.out(1.5)", overwrite: "auto" });
		document.querySelectorAll(".atalho-menu-lobby,.botao-lobby,.botao-menu-lobby,.rpg-sistema-botao").forEach(function (botao) {
			if (botao.dataset.gsapHover === "1") return;
			botao.dataset.gsapHover = "1";
			botao.addEventListener("mouseenter", function () { gsap.to(botao, { scale: 1.025, duration: 0.16, overwrite: "auto" }); });
			botao.addEventListener("mouseleave", function () { gsap.to(botao, { scale: 1, duration: 0.18, overwrite: "auto" }); });
		});
	}

	function ativarHowlerPremium() {
		if (!cdnPremium.howler || window.RpgHowler) return;
		window.RpgHowler = {
			tocar: function (tipo) {
				if (!audioUiLiberado) return;
				tocarUi(tipo || "confirmar");
			},
			disponivel: true
		};
	}
	...
	function mostrarBannerCdn(titulo, texto) {
		...
		banner.id = "rpg-cdn-banner";
		banner.className = "rpg-cdn-banner animate__animated animate__fadeInDown";
		banner.innerHTML = "<i>!</i><span><strong>" + titulo + "</strong><span>" + texto + "</span></span><button type='button'>OK</button>";
		document.body.appendChild(banner);
		banner.querySelector("button").onclick = function () { banner.remove(); };
		setTimeout(function () { if (banner.isConnected) banner.remove(); }, 5200);
	}
	...
	function abrirBoasVindasCdn() {
		...
		if (cdnPremium.sweetalert) {
			...
			mostrarBannerCdn("Modo RPG Premium", "Camada CDN carregada parcialmente. Efeitos locais continuam ativos.");
		}
	}

	function testarCenaCinematica() {
		soltarConfetti("conquista");
		tocarUi("recompensa");
		if (cdnPremium.gsap) {
			window.gsap.fromTo("body", { filter: "brightness(1.22) saturate(1.35)" }, { filter: "brightness(1) saturate(1)", duration: 0.85, ease: "power2.out" });
		}
		if (cdnPremium.sweetalert) {
			window.Swal.fire({
				title: "Evento Lendario",
				html: "Cena cinematica, particulas, confete, animacao e painel premium ativos.",
				icon: "success",
			...
			mostrarBannerCdn("Evento Lendario", "Cena cinematica disparada com os efeitos disponiveis.");
		}
	}

	function soltarConfetti(tipo) {
		if (typeof window.confetti !== "function") return;
		var base = { particleCount: tipo === "conquista" ? 150 : 80, spread: tipo === "conquista" ? 92 : 62, ticks: 180, scalar: tipo === "conquista" ? 1.05 : 0.92, disableForReducedMotion: true };
		window.confetti({ ...base, origin: { x: 0.18, y: 0.72 }, colors: ["#90ff00", "#36e0ff", "#ffe28a"] });
		if (tipo === "conquista") {
			setTimeout(function () { window.confetti({ ...base, particleCount: 110, origin: { x: 0.82, y: 0.72 }, colors: ["#ffe28a", "#fff7df", "#8f7bff", "#ff6b35"] }); }, 120);
			setTimeout(function () { window.confetti({ ...base, particleCount: 64, origin: { x: 0.5, y: 0.38 }, spread: 120, colors: ["#36e0ff", "#90ff00", "#fff7df"] }); }, 260);
		}
	}
		...
		overlay.className = "rpg-loading rpg-boot-plus";
			overlay.innerHTML =
				'<section class="rpg-loading-card" aria-label="Carregando jogo">' +
					...
					'<div class="rpg-loading-info">' +
						'<small>Inicializando servidor RPG</small>' +
						'<h2>One Piece RPG</h2>' +
						'<div class="rpg-loading-barra"><i></i></div>' +
						'<div class="rpg-loading-etapas"><span>Assets</span><span>Ficha</span><span>Mundo</span></div>' +
	...
	function verificarConquistas() {
		...
		var nomeNormalizado = window.RpgSistemas?.normalizar ? window.RpgSistemas.normalizar(personagem.nome) : String(personagem.nome || "").toLowerCase();
		var aceitas = lerJson("rpg_missoes_aceitas_" + nomeNormalizado, []);
		var batalhas = lerJson("rpg_batalhas_" + nomeNormalizado, []);
		var conquistas = [
			...
			{ id: "missao-aceita", ok: aceitas.length > 0, titulo: "Contrato Assinado", texto: "Aceitou uma missao no quadro.", cor: "#36e0ff" },
			{ id: "primeira-batalha", ok: batalhas.length > 0, titulo: "Primeiro Combate", texto: "Registrou uma vitoria em batalha.", cor: "#ff7b7b" },
			{ id: "equipamento", ok: inventario.some(function (item) { return item.equipado; }), titulo: "Pronto Para Zarpar", texto: "Equipou um item no inventario.", cor: "#90ff00" },
			{ id: "rank-a", ok: (personagem.missoesConcluidas || []).some(function (missao) { return missao.rank === "A"; }), titulo: "Operacao Rank A", texto: "Concluiu uma missao Rank A.", cor: "#ffcf3e" }
		];
	...
	function listarPersonagensLocais() {
		var lista = lerJson("personagensCriados", []);
		var ativo = obterPersonagem();
		if (ativo && ativo.nome && !lista.some(function (item) { return item && item.nome === ativo.nome; })) lista.unshift(ativo);
		return lista.filter(function (item, indice, array) {
			return item && item.nome && array.findIndex(function (outro) { return outro && outro.nome === item.nome; }) === indice;
		});
	}
	...
	function instalarHudGlobal() {
		...
		hud.className = "rpg-hud-global";
		hud.innerHTML =
			'<button class="rpg-hud-avatar" type="button" data-hud="trocar" title="Trocar personagem"></button>' +
			'<div class="rpg-hud-info">' +
				'<div class="rpg-hud-linha"><strong class="rpg-hud-nome" id="rpg-hud-nome">Personagem</strong><span class="rpg-hud-meta" id="rpg-hud-meta">Nv. 1</span></div>' +
				'<div class="rpg-hud-bars"><div class="rpg-hud-bar"><i id="rpg-hud-hp"></i></div><div class="rpg-hud-bar ene"><i id="rpg-hud-ene"></i></div></div>' +
				'<div class="rpg-hud-linha"><span class="rpg-hud-meta" id="rpg-hud-exp">0 EXP</span><span class="rpg-hud-meta" id="rpg-hud-berris">0 B</span></div>' +
			'</div>' +
			'<div class="rpg-hud-acoes">' +
				'<button type="button" data-hud="batalha" title="Batalha">BT</button>' +
				'<button type="button" data-hud="missoes" title="Missões">MS</button>' +
				'<button type="button" data-hud="inventario" title="Inventário">INV</button>' +
				'<button type="button" data-hud="trocar" title="Trocar">TR</button>' +
			'</div>';
		document.body.appendChild(hud);
		hud.addEventListener("click", function (evento) {
			var botao = evento.target.closest("[data-hud]");
			if (!botao) return;
			var acao = botao.dataset.hud;
			if (acao === "trocar") { abrirSelecaoPersonagem(); return; }
			if (acao === "batalha") transicionarPagina("batalha.html");
			if (acao === "missoes") transicionarPagina("missoes.html");
			if (acao === "inventario") transicionarPagina("inventario.html");
		});

		function atualizar() {
			var personagem = obterPersonagem();
			if (!personagem || !personagem.nome) { hud.hidden = true; return; }
			hud.hidden = false;
			var progresso = progressoDo(personagem);
			var estado = estadoNivel(progresso);
			var atributos = personagem.atributosDistribuidos || personagem.atributos || {};
			var vital = Number(atributos.forcaVital || atributos.vitalidade || 80);
			var mental = Number(atributos.forcaMental || atributos.mental || 60);
			var hp = Math.round(260 + vital * 2.1 + Number(estado.nivel || progresso.nivel || 1) * 16);
			var ene = Math.round(110 + mental * 1.2);
			document.getElementById("rpg-hud-nome").textContent = personagem.nome;
			document.getElementById("rpg-hud-meta").textContent = "Nv. " + (estado.nivel || progresso.nivel || 1);
			document.getElementById("rpg-hud-exp").textContent = formatar(estado.expAtual) + "/" + formatar(estado.proximo) + " EXP";
			document.getElementById("rpg-hud-berris").textContent = formatar(progresso.berris || 0) + " B";
			document.getElementById("rpg-hud-hp").style.width = Math.max(8, Math.min(100, (hp / Math.max(1, hp)) * 100)) + "%";
			document.getElementById("rpg-hud-ene").style.width = Math.max(8, Math.min(100, (ene / Math.max(1, ene)) * 100)) + "%";
			var avatar = hud.querySelector(".rpg-hud-avatar");
			var imagem = personagem.imagemPersonagem || personagem.imagem || "";
			if (imagem && avatar.dataset.src !== imagem) {
				avatar.dataset.src = imagem;
				avatar.innerHTML = '<img src="' + imagem + '" alt="">';
			}
			if (!imagem) avatar.innerHTML = "";
		}

		window.addEventListener("storage", atualizar);
		window.addEventListener("rpg:progresso-atualizado", atualizar);
		setInterval(atualizar, 3500);
		atualizar();
	}

	function abrirSelecaoPersonagem() {
		var modal = document.getElementById("rpg-selecao-personagem");
		if (modal) { modal.hidden = false; return; }
		var personagens = listarPersonagensLocais();
		var ativo = obterPersonagem();
		modal = document.createElement("div");
		modal.id = "rpg-selecao-personagem";
		modal.className = "rpg-selecao-personagem";
		modal.innerHTML =
			'<section class="rpg-selecao-card" role="dialog" aria-modal="true" aria-label="Selecionar personagem">' +
				'<header><div><small>Personagens salvos</small><h2>Trocar personagem</h2></div><button type="button" data-fechar> X </button></header>' +
				'<div class="rpg-selecao-lista">' +
					(personagens.length ? personagens.map(function (p) {
						var imagem = p.imagemPersonagem || p.imagem || "";
						var ativoClasse = ativo && ativo.nome === p.nome ? " ativo" : "";
						return '<button type="button" class="' + ativoClasse + '" data-personagem="' + p.nome.replace(/"/g, "&quot;") + '"><i>' + (imagem ? '<img src="' + imagem + '" alt="">' : "") + '</i><span><strong>' + p.nome + '</strong><span>' + (p.raca || p.raça || "Raça") + " / " + (p.classe || "Classe") + '</span></span></button>';
					}).join("") : '<button type="button"><i></i><span><strong>Nenhum personagem local</strong><span>Crie ou entre no personagem primeiro.</span></span></button>') +
				'</div>' +
			'</section>';
		document.body.appendChild(modal);
		modal.querySelector("[data-fechar]").onclick = function () { modal.hidden = true; };
		modal.addEventListener("click", function (evento) { if (evento.target === modal) modal.hidden = true; });
		modal.querySelectorAll("[data-personagem]").forEach(function (botao) {
			botao.onclick = function () {
				var escolhido = personagens.find(function (p) { return p.nome === botao.dataset.personagem; });
				if (!escolhido) return;
				localStorage.setItem("ultimoPersonagem", JSON.stringify(escolhido));
				mostrarToast("Personagem ativo", escolhido.nome, "#36e0ff", "!");
				setTimeout(function () { location.reload(); }, 320);
			};
		});
	}

	function instalarPageRail() {
		if (document.getElementById("rpg-page-rail")) return;
		var paginas = [
			{ href: "lobby.html", sigla: "LB", nome: "Lobby" },
			{ href: "atributos.html", sigla: "AT", nome: "Atributos" },
			{ href: "inventario.html", sigla: "INV", nome: "Inventario" },
		...
		var atual = location.pathname.split("/").pop() || "index.html";
		if (atual === "index.html" || atual === "cadastro.html" || atual === "download.html") return;
		var rail = document.createElement("nav");
		rail.id = "rpg-page-rail";
		rail.className = "rpg-page-rail";
		rail.setAttribute("aria-label", "Navegacao rapida do jogo");
		rail.innerHTML =
			"<div><strong>" + (document.title || "RPG").replace(" - One Piece RPG", "") + "</strong><span>Cliente RPG / navegacao rapida</span></div>" +
			'<div class="rpg-page-links">' +
				paginas.map(function (pagina) {
					var ativo = atual === pagina.href ? " ativo" : "";
					return '<a class="' + ativo + '" href="' + pagina.href + '" title="' + pagina.nome + '">' + pagina.sigla + "</a>";
				}).join("") +
			"</div>";
	...
	function instalarPatchNotes(forcar) {
		...
		modal.innerHTML =
				'<section class="rpg-patch-card" role="dialog" aria-modal="true" aria-label="Novidades da versão">' +
					...
					'<div class="rpg-patch-lista">' +
						'<div class="rpg-patch-item"><strong>Interface clean</strong><span>Menos elementos flutuando na tela, radar limitado às abas jogáveis e HUD mais compacto.</span></div>' +
						'<div class="rpg-patch-item"><strong>Visual menos poluído</strong><span>Brilhos, sombras, hotkeys e avisos foram reduzidos para parecer mais jogo e menos painel cheio.</span></div>' +
						'<div class="rpg-patch-item"><strong>Inventário redesenhado</strong><span>Painel lateral agora inspeciona o item selecionado, sem ocupar espaço com a imagem do personagem.</span></div>' +
						'<div class="rpg-patch-item"><strong>Slots com cara de jogo</strong><span>Ícones por categoria, raridade em destaque, grid compacto e rolagem interna sem barras aparecendo toda hora.</span></div>' +
						...
						'<div class="rpg-patch-item"><strong>Game client V40</strong><span>HUD global mais discreto, radar só em páginas jogáveis e inventário sem poluição visual.</span></div>' +
					...
					'<div class="rpg-patch-item"><strong>Lobby navegável</strong><span>Menu e ajustes recolhidos por padrão, barra de rolagem do lobby e navegação rápida global.</span></div>' +
					'<div class="rpg-patch-item"><strong>Efeitos premium</strong><span>Confete em recompensa/conquista, barra NProgress, partículas locais e camada de partículas CDN em tela cheia.</span></div>' +
					...
					'<div class="rpg-patch-item"><strong>Cena cinematica</strong><span>O SYS ganhou teste de evento lendário com modal, brilho, confete e áudio de interface.</span></div>' +
				"</div>" +
		...
		document.body.appendChild(modal);
		localStorage.setItem(chave, "visto");
		modal.querySelectorAll("[data-patch-fechar]").forEach(function (botao) {
			botao.onclick = function () { modal.remove(); };
		});
	...
	function abrirMenuSistema() {
				...
				'<div class="rpg-sistema-corpo">' +
					...
					'<section class="rpg-sistema-bloco"><h3>Salvamento</h3>' +
						...
						'<div class="rpg-sistema-linha"><span>Rede</span><strong>' + (navigator.onLine ? "Online" : "Offline") + '</strong></div>' +
							'<div class="rpg-sistema-linha"><span>Cache</span><strong>v40</strong></div>' +
					...
					'<section class="rpg-sistema-bloco"><h3>CDN Premium</h3>' +
						'<div class="rpg-sistema-linha"><span>GSAP</span><strong><span class="rpg-cdn-status ' + (cdnPremium.gsap ? "" : "off") + '">' + (cdnPremium.gsap ? "ativo" : "off") + '</span></strong></div>' +
						'<div class="rpg-sistema-linha"><span>Particles</span><strong><span class="rpg-cdn-status ' + (cdnPremium.particles ? "" : "off") + '">' + (cdnPremium.particles ? "ativo" : "off") + '</span></strong></div>' +
							'<div class="rpg-sistema-linha"><span>Modal RPG</span><strong><span class="rpg-cdn-status ' + (cdnPremium.sweetalert ? "" : "off") + '">' + (cdnPremium.sweetalert ? "ativo" : "off") + '</span></strong></div>' +
		...
		modal.querySelector("[data-sistema-fullscreen]").onclick = function () {
			...
			modal.querySelector("[data-sistema-cinematico]").onclick = testarCenaCinematica;
			modal.querySelector("[data-sistema-cdn-reload]").onclick = function () {
				carregarBibliotecasCdn();
				mostrarToast("CDN Premium", "Tentando recarregar bibliotecas visuais.", "#36e0ff", "!");
			};
	...
	function comandosPremium() {
		return [
			...
			{ id: "batalha", titulo: "Batalha", desc: "Abrir arena de combate com recompensa", destino: "batalha.html", acao: "batalha", tecla: "B" },
	...
	function instalarRadarGame() {
		var atual = paginaAtual();
		if (!["mundo.html", "batalha.html", "aventura.html"].includes(atual)) return;
	...
	function instalarQuestDock() {
		var atual = paginaAtual();
		if (!["lobby.html", "mundo.html", "batalha.html", "missoes.html", "aventura.html"].includes(atual)) return;
